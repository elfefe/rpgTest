...
public class OpenSimplexNoise {
	...
	public float eval3_XYBeforeZ(float x, float y, float z)
	{
		// Combine rotation with skew transform.
		float xy = x + y;
		float s2 = xy * 0.211324865405187f;
		float zz = z * 0.288675134594813f;
		float xs = s2 - x + zz, ys = s2 - y + zz;
		float zs = xy * 0.577350269189626f + zz;

		return eval3_Base(xs, ys, zs);
	}

	// Similar to the above, except the Y coordinate should always be the "different" coordinate in your use case.
	public float eval3_XZBeforeY(float x, float y, float z)
	{
		// Combine rotation with skew transform.
		float xz = x + z;
		float s2 = xz * 0.211324865405187f;
		float yy = y * 0.288675134594813f;
		float xs = s2 - x + yy, zs = s2 - z + yy;
		float ys = xz * 0.577350269189626f + yy;

		return eval3_Base(xs, ys, zs);
	}
	
	// 3D OpenSimplex Noise (base which takes skewed coordinates directly).
	private float eval3_Base(float xs, float ys, float zs) {
		...
		// Positions relative to origin point.
		float squishOffsetIns = inSum * SQUISH_CONSTANT_3D;
		float dx0 = xins + squishOffsetIns;
		float dy0 = yins + squishOffsetIns;
		float dz0 = zins + squishOffsetIns;
	...
	public float eval4_XYBeforeZW(float x, float y, float z, float w) {
	...
	public float eval4_XZBeforeYW(float x, float y, float z, float w) {
	...
	public float eval4_XYZBeforeW(float x, float y, float z, float w) {
	...
	private float eval4_Base(float xs, float ys, float zs, float ws) {
		...
		// Positions relative to origin point.
		float squishOffsetIns = inSum * SQUISH_CONSTANT_4D;
		float dx0 = xins + squishOffsetIns;
		float dy0 = yins + squishOffsetIns;
		float dz0 = zins + squishOffsetIns;
		float dw0 = wins + squishOffsetIns;
	...
	private float extrapolate(int xsb, int ysb, int zsb, int wsb, float dx, float dy, float dz, float dw)
	{
		...
		return (float) (grad.dx * dx + grad.dy * dy + grad.dz * dz + grad.dw * dw);
	}
	...
	private static final float N3 = 26.92263139946168f;
	private static final float N4 = 8.881759591352166f;
	...
	static {
		...
		Grad3[] grad3 = {
			new Grad3(-1.4082482904633333f,    -1.4082482904633333f,    -2.6329931618533333f),
			new Grad3(-0.07491495712999985f,   -0.07491495712999985f,   -3.29965982852f),
			new Grad3( 0.24732126143473554f,   -1.6667938651159684f,    -2.838945207362466f),
			new Grad3(-1.6667938651159684f,     0.24732126143473554f,   -2.838945207362466f),
			new Grad3(-1.4082482904633333f,    -2.6329931618533333f,    -1.4082482904633333f),
			new Grad3(-0.07491495712999985f,   -3.29965982852f,         -0.07491495712999985f),
			new Grad3(-1.6667938651159684f,    -2.838945207362466f,      0.24732126143473554f),
			new Grad3( 0.24732126143473554f,   -2.838945207362466f,     -1.6667938651159684f),
			new Grad3( 1.5580782047233335f,     0.33333333333333337f,   -2.8914115380566665f),
			new Grad3( 2.8914115380566665f,    -0.33333333333333337f,   -1.5580782047233335f),
			new Grad3( 1.8101897177633992f,    -1.2760767510338025f,    -2.4482280932803f),
			new Grad3( 2.4482280932803f,        1.2760767510338025f,    -1.8101897177633992f),
			new Grad3( 1.5580782047233335f,    -2.8914115380566665f,     0.33333333333333337f),
			new Grad3( 2.8914115380566665f,    -1.5580782047233335f,    -0.33333333333333337f),
			new Grad3( 2.4482280932803f,       -1.8101897177633992f,     1.2760767510338025f),
			new Grad3( 1.8101897177633992f,    -2.4482280932803f,       -1.2760767510338025f),
			new Grad3(-2.6329931618533333f,    -1.4082482904633333f,    -1.4082482904633333f),
			new Grad3(-3.29965982852f,         -0.07491495712999985f,   -0.07491495712999985f),
			new Grad3(-2.838945207362466f,      0.24732126143473554f,   -1.6667938651159684f),
			new Grad3(-2.838945207362466f,     -1.6667938651159684f,     0.24732126143473554f),
			new Grad3( 0.33333333333333337f,    1.5580782047233335f,    -2.8914115380566665f),
			new Grad3(-0.33333333333333337f,    2.8914115380566665f,    -1.5580782047233335f),
			new Grad3( 1.2760767510338025f,     2.4482280932803f,       -1.8101897177633992f),
			new Grad3(-1.2760767510338025f,     1.8101897177633992f,    -2.4482280932803f),
			new Grad3( 0.33333333333333337f,   -2.8914115380566665f,     1.5580782047233335f),
			new Grad3(-0.33333333333333337f,   -1.5580782047233335f,     2.8914115380566665f),
			new Grad3(-1.2760767510338025f,    -2.4482280932803f,        1.8101897177633992f),
			new Grad3( 1.2760767510338025f,    -1.8101897177633992f,     2.4482280932803f),
