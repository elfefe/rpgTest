...
object SimplexNoise {
    ...
    val SKEWING_FACTOR = 0.366025 // 0.5 * ( std::sqrt( 3.0 ) - 1.0 )
    val UNSKEWING_FACTOR = 0.211325 // ( 3.0 - std::sqrt( 3.0 )) / 6.0
     ...
     */
    fun brownianMotionNoise(xPos: Int, yPos: Int, octaves: Int, lacunarity: Double, gain: Double): Double {
        var sum = 0.0
        var frequency = 1.0
        var amplitude = 1.0
        var maxValue = 0.0 // Used for normalizing result between -1.0 and 1.0
        for (i in 0 until octaves) {
            sum += noise(xPos * frequency, yPos * frequency) * amplitude
            maxValue += amplitude
            amplitude *= gain
            frequency *= lacunarity
        }
        return (sum / maxValue) / 2.0 + 0.5
    }
